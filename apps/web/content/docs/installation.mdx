...
| shadcn/ui | registry item | `npx shadcn@latest add https://iconmind.dev/r/iconmind.json` |
...
## For a shadcn project

```bash
npx shadcn@latest add https://iconmind.dev/r/iconmind.json
```

Adds `@iconmind/react` and an `<Icon name="agent-run" />` wrapper in `components/ui/`. See
[shadcn/ui](/docs/shadcn/).
