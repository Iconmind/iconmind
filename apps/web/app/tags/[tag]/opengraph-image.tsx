...
import { allTags, tagOf, titleCase } from "@/lib/index-pages";
...
/** A tag's card: its name and eight of its icons, so the link shows what the page holds. */
export default async function Image({ params }: { params: Promise<{ tag: string }> }) {
  const { tag } = await params;
  const t = tagOf(tag)!;
  const sample = t.icons.slice(0, 8);
    ...
    (
      <div style={{ width: "100%", height: "100%", display: "flex", flexDirection: "column", justifyContent: "space-between", background: "#FAF8F5", padding: 72, fontFamily: "sans-serif" }}>
        <div style={{ display: "flex", flexDirection: "column", gap: 14 }}>
          <div style={{ display: "flex", fontSize: 26, color: "#C2410C", fontWeight: 600, letterSpacing: 2, textTransform: "uppercase" }}>IconMind · tag</div>
          <div style={{ display: "flex", fontSize: 80, fontWeight: 700, color: "#14110E", letterSpacing: -3 }}>{titleCase(t.slug)} icons</div>
          <div style={{ display: "flex", fontSize: 28, color: "#5C554D" }}>{t.count} icons carry this tag — outline and duotone, three weights, MIT</div>
        </div>
