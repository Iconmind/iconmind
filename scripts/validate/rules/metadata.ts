...
export function checkMetadata(icons: IconFile[], r: Report) {
  ...
  for (const [alias, file] of aliases) {
    ...
    // Every alias is exported as a component, so one that cannot be an identifier does not
    // fail here — it fails in esbuild, after the icons are written and everything else is
    // green. `2fa` became `export const 2fa`, which is where this check comes from.
    const comp = toComponentName(alias);
    if (!/^[A-Za-z_$][A-Za-z0-9_$]*$/.test(comp))
      r.add(file, "naming/alias-not-identifier",
        `alias '${alias}' becomes '${comp}', which is not a name JavaScript can export — put it in \`keywords\` instead`);
    if (RESERVED_WORDS.has(comp)) r.add(file, "naming/component-name-safe", `alias '${alias}' becomes '${comp}', which collides with a JS global`);
  }
}
