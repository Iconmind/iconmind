...
- **Frame/rect radius**: r is 2, or h/2 for a capsule. Stub minimum 5×5 at r2.
  Frame gap lips ≥ 2.5.
- Deliberate near-pairs stay: MCP chamfer convention, direction pairs Round 19 adds chat-forward/chat-reply (0.88, mirrored arrow in the same bubble) and grant/least-privilege (0.88, plus/minus beside the same key). Round 21 adds discount/top-up (0.88, minus/plus beside the same coin). Round 24 adds add-goal/remove-goal (0.90, plus/minus beside the same target).
