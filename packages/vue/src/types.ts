...
export interface IconProps<V extends Variant = Variant> extends /* spreads to <svg> */ SVGAttributes {
  ...
  color?: string;
  /**
   * Which drawing to use. Default "outline". Narrowed per icon to the variants that
   * exist, so asking an icon for a drawing it does not have is a compile error.
   */
  variant?: V;
